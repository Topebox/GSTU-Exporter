...
namespace GoogleSheetsToUnity
{
    ...
    public partial class SpreadsheetManager
    {
        ...
        public static void Read(GSTU_Search search, UnityAction<GstuSpreadSheet> callback,
            bool containsMergedCells = false)
        {
        ...
        /// <param name="callback"></param>
        /// <returns></returns>
        static IEnumerator Read(UnityWebRequest request, GSTU_Search search, bool containsMergedCells,
            UnityAction<GstuSpreadSheet> callback)
        {
            if (Application.isPlaying)
            {
            ...
            using (request)
            {
                ...
                callback?.Invoke(new GstuSpreadSheet(responce, search.worksheetName, search.titleColumn,
                    search.titleRow));
            }
        }
